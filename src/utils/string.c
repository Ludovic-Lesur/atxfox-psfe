 ...
 */
void STRING_ConvertValue(unsigned int value, STRING_Format format, unsigned char print_prefix, char* string) {
    // Local variables.
	...
	case STRING_FORMAT_HEXADECIMAL:
		...
		for (idx=3 ; idx>=0 ; idx--) {
			...
			if ((first_non_zero_found != 0) || (idx == 0)) {
				string[string_idx++] = STRING_HexaToAscii((current_value & 0xF0) >> 4);
				string[string_idx++] = STRING_HexaToAscii(current_value & 0x0F);
			}
			if (idx == 0) {
				break;
			}
